          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            >
              <div className="inline-flex items-center gap-2 text-rose-500 dark:text-rose-400 mb-4">
                ...
                <span className="text-sm font-semibold tracking-wider">{content?.controlSectionTitle || 'Tú tienes el control'}</span>
              </div>
              ...
              <p className="text-xl text-slate-600 dark:text-slate-300 leading-relaxed">
                {content?.controlSectionText || 'Personaliza tu invitación desde tu propio panel: agrega fotos, música, ubicación y todos los detalles de tu boda sin complicaciones.'}
              </p>

              <motion.button
                onClick={() => window.open(content?.demoUrl || 'https://suspiro-nupcial.vercel.app/invitacion/humberto-nelida', '_blank')}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              ...
              >
                <span>{content?.demoCtaText || '💍 Mira cómo se verá tu invitación'}</span>
            ...
            {/* Preview Visual (Sin iframe, usando imagen de demo mockeada) */}
            <motion.div
              initial={{ opacity: 0, x: 30 }}
              whileInView={{ opacity: 1, x: 0 }}
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {(content?.testimonialsList || []).map((testimonial, idx) => (
              <motion.div
                ...
                className="bg-slate-50 dark:bg-slate-700/50 p-8 rounded-3xl shadow-lg hover:shadow-xl border border-slate-100 dark:border-slate-600 transition-all cursor-pointer relative"
              >
                <div className="flex mb-4">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <svg key={i} className={`w-5 h-5 ${(testimonial.rating || 5) > i ? 'text-amber-500' : 'text-slate-200 dark:text-slate-600'}`} fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                  ))}
                </div>
                <p className="text-slate-600 dark:text-slate-300 italic mb-6 text-lg">"{testimonial.text}"</p>
                <div className="flex items-center gap-4 mt-auto">
                  {testimonial.avatarUrl && (
                    <img src={testimonial.avatarUrl} alt={testimonial.name} className="w-12 h-12 rounded-full object-cover shadow-sm bg-slate-200" />
                  )}
                  <div>
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {['basic', 'premium', 'deluxe'].map((key, index) => {
              ...
              return (
                ...
                >
                  {key === 'premium' && (
                    <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-rose-400 to-amber-500 text-white text-sm font-bold px-6 py-2 rounded-full shadow-lg z-30 flex items-center gap-1">
                      {content?.pricingHighlightLabel || '⭐ Más elegido'}
                    </div>
                  )}
                  <div className={`relative bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-3xl p-8 shadow-2xl hover:shadow-3xl border-2 transition-all duration-500 overflow-hidden ${selectedPlan === key
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="text-center mb-8">
                        ...
                        <div className="text-sm text-slate-500 dark:text-slate-400 space-y-2 mt-4">
                          <p 
                            className="cursor-help flex items-center justify-center gap-1 group/tooltip relative inline-flex mx-auto"
                          >
                            Tiempo de publicación: {plan.duration} días
                            <svg className="w-4 h-4 text-slate-400 group-hover/tooltip:text-rose-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            {/* Tooltip Popup */}
                            <span className="invisible opacity-0 group-hover/tooltip:visible group-hover/tooltip:opacity-100 bg-slate-800 text-white text-xs w-64 p-3 rounded-xl absolute bottom-full mb-2 left-1/2 -translate-x-1/2 z-50 shadow-xl transition-all duration-300 pointer-events-none">
                              Tu sitio web estará público y accesible para tus invitados durante el tiempo seleccionado. Pasado este período, puedes renovarlo.
                          ...
                          <p className="font-medium bg-slate-100 dark:bg-slate-700/50 py-1 px-3 rounded-full inline-block mt-1">
                            {plan.maxGuests >= 999999 ? 'Invitados ILIMITADOS' : `Hasta ${plan.maxGuests} invitados`}
                          </p>
                        </div>
                      </div>
