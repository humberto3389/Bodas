...
export function MediaManager({
    ...
    return (
        ...
        >
            {/* Sección de Imágenes */}
            <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-6 sm:p-8 shadow-xl border border-white/40">
                <h2 className="text-2xl font-brush bg-gradient-to-r from-rose-600 to-amber-600 bg-clip-text text-transparent mb-6">
                    Galería de Imágenes
                </h2>
                <AdminUploader
            ...
            <div className={`bg-white/90 backdrop-blur-sm rounded-3xl p-6 sm:p-8 shadow-xl border border-white/40`}>
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h2 className="text-2xl font-brush bg-gradient-to-r from-rose-600 to-amber-600 bg-clip-text text-transparent">
                    ...
                    {effectivePlan === 'deluxe' && (
                        <div className="flex items-center gap-3 bg-white/50 px-4 py-2 rounded-full border border-white/60">
                            <span className="text-sm text-slate-600 font-medium">Audio en Video</span>
                            <button
                                onClick={() => onToggleAudio(!cinemaVideoAudioEnabled)}
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${cinemaVideoAudioEnabled ? 'bg-green-500' : 'bg-slate-300'}`}
                            >
